...
const RenterChatManagement = () => {
  ...
  return (
    <div className={`min-h-screen bg-slate-50/50 ${selectedConversation ? "py-0 px-0 md:py-8 md:px-4" : "py-8 px-4"} sm:px-6 lg:px-8`}>
      <div className={`max-w-7xl mx-auto flex flex-col ${selectedConversation ? "h-[100vh] md:h-[calc(100vh-8rem)] min-h-0 md:min-h-[600px] md:max-h-[780px]" : "h-[calc(100vh-8rem)] min-h-[600px] max-h-[780px]"}`}>
        
        {/* Page Title Header */}
        <div className={`flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4 shrink-0 ${selectedConversation ? "hidden md:flex" : "flex"}`}>
          <div>
        ...
        <div className={`flex-1 flex bg-white ${selectedConversation ? "border-0 md:border md:border-slate-200/85 rounded-none md:rounded-2xl shadow-none md:shadow-xl" : "border border-slate-200/85 rounded-2xl shadow-xl"} overflow-hidden min-h-0`}>
          
          {/* LEFT SIDEBAR: Conversational List */}
          <div className={`w-full md:w-80 border-r border-slate-150 flex flex-col bg-slate-50/30 shrink-0 ${selectedConversation ? "hidden md:flex" : "flex"}`}>
          ...
          <div className={`flex-1 flex flex-col bg-white min-w-0 ${selectedConversation ? "flex" : "hidden md:flex"}`}>
                ...
                <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-white shrink-0">
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <button
                      ...
                      className="p-1.5 -ml-1 text-slate-500 hover:text-slate-800 rounded-lg hover:bg-slate-100 transition-colors md:hidden shrink-0"
                    >
                    ...
                    <div className="min-w-0 flex-1">
                      <h2 className="text-base font-bold text-slate-800 leading-tight flex items-center gap-1.5">
                        <span className="truncate">{selectedConversation.renterName}</span>
                  ...
                  <div className="flex items-center gap-1">
                    {selectedConversation.renterPhone && (
                      <a
                        href={getWhatsAppLink(selectedConversation.renterPhone)}
                        target="_blank"
                        rel="noopener noreferrer"
                        title="Chat on WhatsApp"
                        className="p-2 text-slate-400 hover:text-emerald-500 hover:bg-emerald-50 rounded-xl transition-all border border-transparent hover:border-emerald-100 flex items-center justify-center"
                      >
                        <svg
                          className="w-5 h-5 fill-current text-emerald-500 hover:text-emerald-600"
                          viewBox="0 0 24 24"
                        >
                    ...
                    <button
                      ...
                      className="p-2 text-slate-400 hover:text-blue-600 hover:bg-slate-50 rounded-xl transition-all border border-transparent hover:border-slate-100 flex items-center justify-center"
                    >
