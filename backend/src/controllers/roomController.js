...
export const createRoom = async (req, res) => {
    try {
        const { roomNumber, title, description, price, roomType, bookingType, floor, capacity, images, amenities, } = req.body;
...
export const getRooms = async (req, res) => {
    try {
        ...
        const roomsWithParsedData = rooms.map(room => {
            let currentRenterName = undefined;
...
export const updateRoom = async (req, res) => {
    try {
        const { id } = req.params;
        const { roomNumber, title, description, price, roomType, bookingType, floor, capacity, isAvailable, images, amenities, } = req.body;
        const room = await prisma.room.findUnique({
            where: { id: Number(id) },
        });
        if (!room) {
