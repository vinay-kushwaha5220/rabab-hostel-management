...
export function getCycleMonthString(start, end) {
...
export function calculateCycleEnd(start) {
...
export const createBooking = async (req, res) => {
    try {
        ...
        const { roomId, customerName, customerEmail, customerPhone, customerAadhaar, checkInDate, checkOutDate, numberOfGuests, bookingType, // DAILY or MONTHLY
         } = req.body;
...
// ==========================================
export const undoCheckOutBooking = async (req, res) => {
    const { id } = req.params;
    try {
        const booking = await prisma.booking.findUnique({
            where: { id: Number(id) },
...
// ==========================================
export const getMonthlyActiveBookings = async (req, res) => {
    try {
...
// ==========================================
export const renewMonthlyStay = async (req, res) => {
    const { id } = req.params;
    try {
...
// ==========================================
export const extendDailyBooking = async (req, res) => {
    const { id } = req.params;
...
// ==========================================
export const createBookingRazorpayOrder = async (req, res) => {
    try {
        ...
        const amountInPaise = Math.round(booking.totalAmount * 100); // Razorpay requires amount in Paise (INR * 100)
        const options = {
        ...
        console.log(`Creating Razorpay Order for Booking ID ${bookingId}, Amount: ₹${booking.totalAmount}...`);
        ...
        console.error("Create Booking Razorpay order error:", error);
        res.status(500).json({
...
// ==========================================
export const verifyBookingRazorpayPayment = async (req, res) => {
    try {
        const { bookingId: rawBookingId, razorpay_order_id, razorpay_payment_id, razorpay_signature } = req.body;
        const userId = req.userId;
        if (!userId) {
            return res.status(401).json({
                message: "Unauthorized - User session not found",
        ...
        const bookingId = Number(rawBookingId);
        if (!bookingId || isNaN(bookingId) || !razorpay_order_id || !razorpay_payment_id || !razorpay_signature) {
            return res.status(400).json({
        ...
        if (!isSignatureValid) {
            console.error(`❌ Razorpay signature verification FAILED for Booking ID ${bookingId}`);
            return res.status(400).json({
                message: "Payment signature verification failed. Unauthorized transaction.",
            });
        }
        console.log(`✅ Razorpay signature verified successfully for Booking ID ${bookingId}, Payment ID ${razorpay_payment_id}`);
        // Update the booking status and billing (wrapped in transaction)
        await prisma.$transaction(async (tx) => {
            ...
            // 2. Confirm booking and set paymentStatus to SUCCESS
            await tx.booking.update({
                where: { id: booking.id },
                data: {
                    status: BookingStatus.CONFIRMED,
            ...
            // 3. If MONTHLY stay, initialize cycles & first month's bill as PAID
            if (booking.bookingType === BookingType.MONTHLY) {
                const joinDate = new Date(booking.checkInDate);
                const firstCycleEnd = calculateCycleEnd(joinDate);
                const rentAmount = booking.room?.monthlyPrice || (booking.room?.dailyPrice * 30) || (booking.room?.price * 30) || 0;
            ...
            await tx.notification.create({
                data: {
                    bookingId: booking.id,
                    title: "Online Booking Payment Success",
                    message: `Online booking payment of ₹${booking.totalAmount} received successfully via Razorpay (Payment ID: ${razorpay_payment_id}). Booking ${booking.bookingId} is confirmed.`,
                    type: NotificationType.PAYMENT,
                    priority: NotificationPriority.HIGH,
                },
            });
        ...
        console.error("Verify Booking Razorpay payment error:", error);
        res.status(500).json({
