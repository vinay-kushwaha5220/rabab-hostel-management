...
const NotificationsPage = () => {
  ...
  const getNotificationIconStyles = (type: string, isRead: boolean) => {
    if (isRead) {
    ...
    switch (type) {
      ...
      default:
        return "bg-blue-50 border border-blue-100 text-blue-650"
    }
  }
    ...
    <div className="min-h-screen bg-slate-50 p-4 sm:p-6 lg:p-8 relative">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
          <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow transition-shadow">
          ...
          {filteredNotifications.length === 0 ? (
            ...
            filteredNotifications.map((notif) => (
              <div
                ...
                className={`bg-white p-5 rounded-2xl border transition-all hover:shadow-md cursor-pointer flex items-center justify-between gap-4 group ${
                  notif.isRead ? "border-slate-150/80" : "border-blue-200/70 bg-blue-50/15 hover:bg-blue-50/25"
                }`}
              >
                <div className="flex items-start gap-4 flex-1 min-w-0">
                  {/* Decorative Icon inside themed soft frame */}
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm transition-transform group-hover:scale-105 ${
                    getNotificationIconStyles(notif.type, notif.isRead)
                  }`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      ...
                      {!notif.isRead && (
                        <span className="px-2 py-0.5 bg-blue-50 border border-blue-100 text-blue-600 text-[10px] font-bold rounded-full uppercase tracking-wider">
                          New
                        </span>
                      )}
                      <span className={`px-2.5 py-0.5 border rounded-full text-[10px] font-bold uppercase tracking-wider ${getNotificationBadgeColor(notif.type)}`}>
                        {notif.type}
                      </span>
                    </div>
                    ...
                    {/* Booking/Room details badges */}
                    {notif.booking && (
                      <div className="flex flex-wrap items-center gap-2 mt-3.5">
                        <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-50 text-slate-500 text-[11px] font-semibold border border-slate-100/80 shadow-sm shadow-slate-50">
                          <User className="w-3.5 h-3.5 text-slate-400" />
                          Renter: <span className="text-slate-800 font-bold">{notif.booking.customerName}</span>
                        </span>
                        <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-50 text-slate-500 text-[11px] font-semibold border border-slate-100/80 shadow-sm shadow-slate-50">
                          <Building className="w-3.5 h-3.5 text-slate-400" />
                          Room: <span className="text-slate-800 font-bold">{notif.booking.room.roomNumber}</span>
                        </span>
                        <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-50 text-slate-500 text-[11px] font-semibold border border-slate-100/80 shadow-sm shadow-slate-50 font-mono">
                          <Hash className="w-3.5 h-3.5 text-slate-400" />
                          ID: <span className="text-slate-850 font-bold">{notif.booking.bookingId}</span>
                        </span>
                      </div>
                    )}
                  </div>
                ...
                <div className="flex items-center gap-4 flex-shrink-0" onClick={(e) => e.stopPropagation()}>
                  <div className="hidden sm:block text-right">
                    <span className="text-[10px] text-slate-450 font-bold uppercase tracking-wider font-mono bg-slate-50 border border-slate-150/70 px-2.5 py-1 rounded-lg shadow-sm">
                  ...
                  {!notif.isRead && (
                    <button
                      ...
                      className="text-xs font-bold uppercase tracking-wider text-blue-600 hover:text-blue-700 bg-blue-50 hover:bg-blue-100 px-3.5 py-2 rounded-xl active:scale-95 transition-all cursor-pointer shadow-sm shadow-blue-100/40"
                    >
              ...
              {selectedNotif.booking ? (
                <>
                  ...
                  {selectedNotif.booking.monthlyBills && selectedNotif.booking.monthlyBills.some(b => b.status === "VERIFICATION_PENDING") && (
                    <div className="space-y-3 bg-indigo-50/50 border border-indigo-100 rounded-2xl p-5">
