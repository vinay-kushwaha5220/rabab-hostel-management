              ...
              {activeBooking ? (
                <div className="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm relative overflow-hidden group">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    {activeBooking.bookingType === 'MONTHLY' && activeBooking.monthlyRenter && (
                      <div className="space-y-1 bg-amber-50/20 p-4 rounded-xl border border-amber-100 sm:col-span-2">
                        <p className="text-[9px] font-bold text-amber-500 uppercase tracking-wider flex items-center gap-1.5">
                          <Bell size={11} className="text-amber-500 animate-bounce" /> Next statement due date
                        </p>
                        <p className="text-base font-bold text-amber-600 mt-0.5">
                          {formatDate(activeBooking.monthlyRenter.dueDate)}
                        </p>
                        <p className="text-[10px] text-slate-500 font-medium">
                          Your next monthly cycle billing statement will automatically generate at the end of current cycle.
                        </p>
                      </div>
                    )}
